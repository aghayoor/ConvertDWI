...
tic
[X_L2, cost] = OpWeightedL2(b,ones(res),lambda,A,At,res,Niter,tol,gam);
toc
% Show a 2D slice
X_L2_size = size(X_L2);
X_L2_2d = X_L2(:,:,round(X_L2_size(3)/2));
figure(8); imagesc(abs(X_L2_2d),[0,1]); colorbar; title('hi resolution, L2');
figure(9); plot(cost); xlabel('iteration'); ylabel('cost');
figure(10); imagesc(abs(X_L2_2d-X0_2d),[0,0.2]); colorbar; title('L2 error image');

SNR_L2 = -20*log10(norm(X_L2(:)-X0(:))/norm(X0(:)));
...
figure(11); imagesc(abs(X_WL2_2d),[0,1]); colorbar; title('hi resolution, WL2');
figure(12); plot(cost); xlabel('iteration'); ylabel('cost');
figure(13); imagesc(abs(X_WL2_2d-X0_2d),[0,0.2]); colorbar; title('WL2 error image');
...
figure(14); imagesc(abs(X_TV_PD_2d),[0,1]); colorbar; title('hi resolution, TV_PD');
figure(15); plot(cost); xlabel('iteration'); ylabel('cost');
figure(16); imagesc(abs(X_TV_PD_2d-X0_2d),[0,0.2]); colorbar; title('TV_PD error image');
...
Niter = 400;  %number of iterations (typically 500-1000 for Fourier inversion)
gam = 0.01; %set in the range [0.01,1].
Xinit = real(At(b)); %initialization
tic
...
figure(17); imagesc(abs(X_WTV_2d),[0,1]); colorbar; title('hi resolution, WTV');
figure(18); plot(cost); xlabel('iteration'); ylabel('cost');
figure(19); imagesc(abs(X_WTV_2d-X0_2d),[0,0.2]); colorbar; title('WTV error image');
