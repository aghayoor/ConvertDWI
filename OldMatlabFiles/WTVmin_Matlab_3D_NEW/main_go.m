...
lambda = 1e-5; %regularization parameter (typically in the range [1e-2,1], if original image scaled to [0,1])
...
Niter = 100;  %number of iterations (typically 500-1000 for Fourier inversion)
siz = size(edgemask);
edgemask_1 = ones(siz);
tic
[X_TV_PD2, cost] = OpWeightedTV_PD_AHMOD2(b,edgemask_1,lambda,A,At,res,Niter);
toc
% Show a 2D slice of X_TV image
X_TV_PD2_size = size(X_TV_PD2);
X_TV_PD2_2d = X_TV_PD2(:,:,round(X_TV_PD2_size(3)/2));
