...
% HCP test data
load /scratch/TESTS/IpythonNotebook/20160615_HCPWF/2_SRWF/test_tune_parameters/matlabFiles/dwib0_testdata;
...
% HCP mask
load /scratch/TESTS/IpythonNotebook/20160615_HCPWF/2_SRWF/test_tune_parameters/matlabFiles/edgemask_t1t2_1ByGMI.mat
...
figure(2); imagesc(edgemask,[0 1]); colorbar; title('spatial weights image');
...
siz = size(edgemask);
edgemask_1 = ones(siz);
[X_TV, cost] = OpWeightedTV_PD_AHMOD2(b,edgemask_1,lambda,A,At,res,Niter);
figure(5); imagesc(abs(X_TV),[0,1]); colorbar; title('hi resolution, TV (no edgemask)');
figure(6); plot(cost); xlabel('iteration'); ylabel('cost');
SNR_TV = -20*log10(norm(X_TV(:)-X0(:))/norm(X0(:)));
fprintf('TV output SNR = %2.1f dB\n',SNR_TV);
...
lambda = 5e-3; %regularization parameter (typically in the range [1e-2,1], if original image scaled to [0,1])
